...
function GenreMoviePage() {
    ...
    try {
        const arrPath = pathname?.split('/movies/')
        idV = arrPath[1]
    ...
    useEffect(() => {
        setIsLoading(true);
        dispatch(fetchDataByGenre({ genre: idGenre, type: 'movie', pageNumber: pageNumber })).then(() => setIsLoading(false))
    }, [pageNumber])
        ...
        <>
            ...
            <main className='pl-4 md:pl-40 lg:pl-48 h-full w-full bg-gradient-to-b from-redDark/50 via-light from-5% via-20% to-light dark:bg-dark dark:bg-gradient-to-b dark:from-redDark/50 dark:via-dark dark:from-5% dark:via-20% dark:to-dark'>
                ...
                <div className="relative pr-4">
                    <CardSlider title={`${nameGenre} Movies`} data={movies} type_media='movie' list_liked={true}
